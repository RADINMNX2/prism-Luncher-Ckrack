...
void LaunchController::decideAccount()
{
    ...
    if (accounts->count() == 0) {
        ...
        auto reply = CustomMessageBox::selectable(m_parentWidget, tr("No Accounts"),
                                                  tr("In order to play Minecraft, you need to add at least one account. "
                                                     "You can use a Microsoft account or a cracked (offline) account. "
...
LaunchDecision LaunchController::decideLaunchMode()
{
    ...
    // Cracked (offline) accounts never need a Microsoft account or token refresh.
    // They always launch in offline mode, which is how cracked servers work.
